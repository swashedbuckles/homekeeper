...
describe('Household Basic Routes (Integration)', () => {
  describe('GET /', () => {
  ...
  describe('POST /', () => {
    ...
    it('should create a household with valid data', async () => {
      ...
      const savedHousehold = await Household.findById(response.body.data.id);
  ...
  describe('GET /:id', () => {
    it('should return household when user is a member', async () => {
      ...
      expect(response.status).toBe(HTTP_STATUS.OK);
      expect(response.body.data.id).toBe(household._id.toString());
